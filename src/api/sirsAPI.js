...
const sirsApi = axios.create({
  baseURL: "https://salones-api-production.up.railway.app/api/v1",
});

export const postSalon = async ({ name, capacity, isActive }) => {
  const res = await sirsApi.post("/salones", {
    name: name,
    capacity: capacity,
    isActive: isActive
  });
